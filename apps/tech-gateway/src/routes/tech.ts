...
router.post("/message", async (req, res) => {
  try {
    ...
    const r = await (global as any).fetch(brainUrl + "/v1/repairbot/message", {
      method: "POST",
...
router.patch("/jobs/:id/status", async (req, res) => {
  try {
    const brainUrl = process.env.BRAIN_API_URL || "http://localhost:8081";
    const r = await (global as any).fetch(`${brainUrl}/v1/jobs/${encodeURIComponent(req.params.id)}/status`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(req.body || {})
    });
