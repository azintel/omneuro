...
router.get('/stream', (req,res)=>{
  ...
  if(!techId){ res.status(400).end(); return; }
  ...
  const timer = setInterval(()=>{
    ...
    if(items.length){ since = items[items.length-1].created_at.replace('T',' '); send({ type:'append', items }); }
  }, 1000);
...
router.post('/reply', (req,res)=>{
  ...
  if(!tech){ res.status(404).json({ ok:false }); return; }
  const mid = nid();
